...
let projects = [
  {
    image: "textures/project-virtualtrige.png",
    url: "https://virtualtriage.ca/",
  },
  {
    image: "textures/lawfully.png",
    url: "https://lawfully.dev-bt.xyz/",
  },
  {
...
const fanLight3 = new THREE.PointLight(0xffff00, 30, 0.2);
const fanLight4 = new THREE.PointLight(0xff0000, 30, 0.2);
...
function loadIntroText() {
  ...
  const imageTexture = textureLoader.load("textures/project-virtualtrige.png");

  loader.load("fonts/unione.json", function (font) {
    const textMaterials = [
      new THREE.MeshPhongMaterial({ color: 0x171f27, flatShading: true }),
      new THREE.MeshPhongMaterial({ map: imageTexture }), // Apply the texture to the text material
    ];

    const titleGeo = new TextGeometry("Usama Khan", {
      font: font,
      size: 0.08,
      height: 0.01,
    });

    titleText = new THREE.Mesh(titleGeo, textMaterials, imageTexture);
  ...
  loader.load("fonts/helvatica.json", function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      "Web Designer / Developer / Content Creator / 3d Website",
      {
        font: font,
        size: 0.017,
        height: 0,
